...
[Open the live decision room](https://sutasmantas.github.io/retention-decisioning/)
...
The credential-free workflow is fully API-backed:
...
### Real randomized-data feasibility benchmark

A separate CLI runs the same decisioning logic on the checksum-pinned
64,000-row Hillstrom email experiment:

```bash
pip install -c requirements-benchmark.txt -e ".[dev]"
python -m signalroom.real_benchmark
```

The command downloads the 64,000-row file from a commit-pinned GitHub source,
validates its SHA-256 and leakage-safe schema, trains a Mens-email versus
control T-learner, and evaluates a 20%-capacity policy on a joint-stratified
holdout. It writes JSON and Markdown evidence under `artifacts/real_data/`.

The target, split, reuse audit, and promotion thresholds were frozen before
the run in [the admission record](docs/REAL_DATA_ADMISSION.md). Learned
targeting is promoted only if held-out Qini, top-capacity uplift, and IPW
policy value all pass; a positive average treatment effect alone is not enough.
The four-package constraints file freezes the numerical stack used by the
committed evidence while the normal product dependency ranges remain usable.
...
[model card](docs/model-card.md) for model, policy, and deployment details.
